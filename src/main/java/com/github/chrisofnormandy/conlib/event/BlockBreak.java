...
public class BlockBreak {
    ...
    public static final void cancelBlockBreak(final BlockEvent.BreakEvent event) {
        ...
        if (ModRegister.blocks_unbreakable.containsKey(blockIn.getName().toString()))
    ...
    public static final void replaceBlock(final BlockEvent.BreakEvent event) {
        ...
        if (ModRegister.blocks_replaceable.containsKey(blockIn.getName().toString())) {

            if (player.hasCorrectToolForDrops(event.getState())) {
                Random rand = new Random();
                event.setCanceled(true);

                Tuple<Block, Block> repl = ModRegister.blocks_replaceable.get(blockIn.getName().toString());

                event.getLevel().setBlock(event.getPos(), repl.y.defaultBlockState(), 1);
