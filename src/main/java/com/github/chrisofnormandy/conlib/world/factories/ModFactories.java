...
public class ModFactories {
        ...
        public static <T extends IArea, C extends IExtendedNoiseRandom<T>> IAreaFactory<T> modBiomeFactory(
                        IAreaFactory<T> landSeaAreaFactory, IAreaFactory<T> climateAreaFactory,
                        LongFunction<C> contextFactory) {
                IAreaFactory<T> biomeFactory = ModBiomeLayer.INSTANCE.run(contextFactory.apply(200L),
                                landSeaAreaFactory, climateAreaFactory);
        ...
        public static <T extends IArea, C extends IExtendedNoiseRandom<T>> IAreaFactory<T> modClimateFactory(
                ...
                return ModClimateLayer.INSTANCE.run(contextFactory.apply(103L),
                                temperatureLayers.get("default").x
                                                .run(contextFactory.apply(temperatureLayers.get("default").y)),
                                rainfallLayers.get("default").x
                                                .run(contextFactory.apply(rainfallLayers.get("default").y)));
        }
        ...
        public static <T extends IArea, C extends IExtendedNoiseRandom<T>> IAreaFactory<T> createAreaFactories(
                ...
                IAreaFactory<T> rivSubFactory = LayerUtil.zoom(1000L, ZoomLayer.NORMAL,
                                StartRiverLayer.INSTANCE.run(contextFactory.apply(100L), worldFactory), 2,
                                contextFactory);
                ...
                IAreaFactory<T> riverFactory = SmoothLayer.INSTANCE.run(contextFactory.apply(1000L),
                                RiverLayer.INSTANCE.run(contextFactory.apply(1L), LayerUtil.zoom(1000L,
                                                ZoomLayer.NORMAL, rivSubFactory, riverSize, contextFactory)));
